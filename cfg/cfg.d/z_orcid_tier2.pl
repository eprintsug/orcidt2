...
$c->{find_orcid_exports} = sub
{
	my( $item, $works, $put_codes ) = @_;
...
$c->{export_work} = sub
{
	my( $repo, $processor, $item_id, $orcid_id, $token, $put_code ) = @_;
	...
	unless ( $work_xml )
	{
		$processor->add_message( "warning", 
			$repo->html_phrase( "Plugin/Screen/User/Orcid/OrcidManager:export_no_work_xml", item=>$repo->make_text( $item_id )  ) );
		return;
	}
...
#print STDERR "\n\n\n\n####### got response [".Data::Dumper::Dumper($response)."]\n\n";
	...
	{
		$processor->add_message( "warning",
			$repo->html_phrase( "Plugin/Screen/User/Orcid/OrcidManager:orcid_export_error", 
...
$c->{plugins}->{"Screen::EPrint::ExportToOrcid"}->{params}->{disable} = 0;
